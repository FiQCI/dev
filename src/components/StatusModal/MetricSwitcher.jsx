...
export const MetricSwitcher = (props) => {
    ...
    const { metricsState, updateMetricsState, calibrationData,
            qubitMetricOptions, couplerMetricOptions, resonatorMetricOptions, hasResonator,
            qubitInputValue, setQubitInputValue,
            couplerInputValue, setCouplerInputValue,
            resonatorInputValue, setResonatorInputValue } = props;
        ...
        <div className='sticky top-4'>
            <div className='overflow-y-auto scroll-smooth max-h-[70vh] pr-4'>
                ...
                {hasResonator && (
                    <>
                        <div className='mt-4'>
                            <p className='font-bold mb-[2px]'>Resonator Metric:</p>
                            <CSelect
                                hideDetails={true}
                                className='py-2'
                                clearable
                                value={metricsState.resonatorMetric}
                                items={resonatorMetricOptions}
                                placeholder={(resonatorMetricOptions?.length ? 'Choose metric' : 'No resonator metrics available')}
                                onChangeValue={(e) => {
                                    updateMetricsState('resonatorMetric', e.detail || '');
                                    updateMetricsState('thresholdResonator', 0);
                                    setResonatorInputValue('0');
                                }}
                            />
                        </div>

                        <div className="w-full">
                            {(metricsState.resonatorMetric) && (() => {
                                const resonatorStats = metricsState.resonatorMetric ? getMetricStatistics(calibrationData, metricsState.resonatorMetric, 3) : null;
                                            ...
                                            <div className="w-full mt-2">
                                                <div className="flex justify-between text-sm mb-1 w-full">
                                                    <div className="flex flex-row items-center">
                                                        ...
                                                        <input
                                                            ...
                                                            step="any"
                                                            className="w-20 px-2 py-1 text-sm border rounded"
                                                            value={resonatorInputValue}
                                                            onChange={(e) => {
                                                                setResonatorInputValue(e.target.value);
                                                                const inputValue = parseFloat(e.target.value);
                                                                if (!isNaN(inputValue)) {
                                                                    ...
                                                                    // Convert display value back to raw value
                                                                    let rawValue = inputValue;
                                                                    if (resonatorStats.unit === 's') {
                                                                        rawValue = inputValue / 1e6; // Convert μs back to s
                                                                    } else if (resonatorStats.unit === '' || resonatorStats.unit === '%') {
                                                                        rawValue = inputValue / 100; // Convert % back to decimal
                                                                    }
                                                                    ...
                                                                    updateMetricsState('thresholdResonator', clampedSliderValue);
                                                                    updateMetricsState('thresholdResonatorValue', rawValue);
                                                                }
                                                            }}
                                                        />
                                                        <span className="text-sm">
                                                            {resonatorStats.unit === 's' ? 'μs' :
                                                                (resonatorStats.unit === '' || resonatorStats.unit === '%') ? '%' :
                                                                    resonatorStats.unit}
                                                        </span>
                                                    </div>
                                                </div>
                                                <div className="relative w-full h-6 rounded my-2"
                                                    style={{
                                                        background: (() => {
                                                            const isLowerBetter = metricsState.resonatorMetric.includes("error");
                                                            let worst = isLowerBetter ? resonatorStats.best : resonatorStats.worst;
                                                            let best = isLowerBetter ? resonatorStats.worst : resonatorStats.best;
                                                            const colors = generateMetricGradient(worst, best, resonatorStats.average);
                                                            const gradientStops = [];
                                                            for (let i = 0; i <= 10; i++) {
                                                                const index = Math.floor((i / 10) * (colors.length - 1));
                                                                const percentage = (i / 10) * 100;
                                                    ...
                                                    <input
                                                        ...
                                                        max="1"
                                                        step="0.01"
                                                        value={metricsState.thresholdResonator}
                                                        onChange={(e) => updateMetricsState('thresholdResonator', parseFloat(e.target.value))}
                                                        className="absolute top-0 left-0 w-full h-full opacity-70 cursor-pointer slider"
                                                        style={{
                                                            background: 'transparent',
                                                            appearance: 'none',
                                                ...
                                                <div className="flex justify-between text-sm mb-1 w-full">
                                                    <span>
                                                        ...
                                                        {(() => {
                                                            const isLowerBetter = metricsState.resonatorMetric.includes("error");
                                                            const worst = isLowerBetter ? resonatorStats.best : resonatorStats.worst;
                                                            return formatMetricValue(worst, resonatorStats.unit);
                                                        })()}
                                                    </span>
                                                    ...
                                                    <span>
                                                        ...
                                                        {(() => {
                                                            const isLowerBetter = metricsState.resonatorMetric.includes("error");
                                                            const best = isLowerBetter ? resonatorStats.worst : resonatorStats.best;
                                                            return formatMetricValue(best, resonatorStats.unit);
                                                        })()}
                                                    </span>
                                                </div>
                                            </div>
